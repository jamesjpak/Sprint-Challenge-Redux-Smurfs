...
Actions are pieces of data usually containing a type property and a payload.  The actions are sent to the reducers and dictate which changes are triggered.  Reducers are pure functions which creates a new and modified version of the state in the store.  The redux store holds all of the state of the application.  The store is known as a ‘single source of truth’ because the redux store is immutable.  


- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state is utilizing a global container for all the state data to be dispersed to necessary components.  In contrast, component state is state limited to a local scope.  


- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Redux thunk is a function that returns another function.  This allows us to perform async operations in the action creators of an application. Furthermore, the axios calls are then made in the action creators.  
